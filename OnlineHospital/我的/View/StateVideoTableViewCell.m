...
@implementation StateVideoTableViewCell
- (void)layoutSubviews{
...
//        [self ];
    ...
    if (_isShowDate) {
        ...
        if (_isReturn) {
            _labAllMoney.text = [ self decimalwithFloatV:_model.normalAmount];
            _labPay.text = [NSString stringWithFormat:@"提交时间: %@",[BaseDataChange getDateStringWithTimeStr:_model.createdDate]];
        }else{
            _labAllMoney.text =  [ self decimalwithFloatV:_orderModel.normalAmount];
...
//FamousInteract:名医视频--互动,FamousAudit:名医视频--旁听,Lecture:专家讲座--旁听,VIP:vip礼包。Recharge:充值
    ...
    NSString *strPay = [NSString stringWithFormat:@"订单金额   ¥%@   ",[self decimalwithFloatV:_orderModel.normalAmount]];
...
- (void)addModelView{
    ...
    self.labMoney.text = [self decimalwithFloatV: _model.paidAmount];
...
//    }
    ...
    NSString *strPay = [NSString stringWithFormat:@"总价 ¥%@   ",[self decimalwithFloatV:_enrollModel.price]];
//    NSString *str =[NSString stringWithFormat:@"总价 ¥%@.00  已退款",model.paidAmount];
//    cell.labPay.attributedText = [self getPriceAttribute:str pay:model.paidAmount];
    ...
    self.labMoney.text =[self decimalwithFloatV:_enrollModel.price];
    switch ([_orderModel.status intValue]) {//0：待支付，1：已支付，2：已消费，3：申请退款，4：已超时，5：已退款
        case 0:
            self.labState.text = @"待支付";
            break;
